...
app.get("/api/stats/player/:playerId", async (req, res) => {
  ...
  try {
    // نبحث عن كل المباريات التي شارك فيها هذا اللاعب
    // بافتراض أن اللاعبين مرتبطون بالفرق فقط
    // إذا أردت المشاركة الفردية داخل مباراة بشكل مباشر، يجب تعديل الـ DB

    // أولاً نحصل على جميع الفرق التي ينتمي إليها اللاعب
    ...
    // ثم نجد كل المباريات التي كان فريقه فيها طرفاً
    const matches = await prisma.match.findMany({
      where: {
        OR: [
          { teamA_id: { in: teamIds } },
    ...
    matches.forEach(match => {
      ...
      const opponentName = playerTeamId === match.teamA_id
        ? (match.teamB ? match.teamB.name : null)
        : (match.teamA ? match.teamA.name : null);
